...
| <a name="input_reserved_concurrent_executions"></a> [reserved\_concurrent\_executions](#input\_reserved\_concurrent\_executions) | Concurrency reserved for the handler. -1 reserves none, which is what a low-quota account needs: AWS keeps a floor of 10 unreserved executions per account, so an account whose "Concurrent executions" quota is the minimum of 10 rejects any reservation at all rather than only a large one. | `number` | `5` | no |
